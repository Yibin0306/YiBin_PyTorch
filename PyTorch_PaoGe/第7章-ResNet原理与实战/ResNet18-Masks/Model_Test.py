...
from Model import ResNet, ResidualBlock  # 从自定义的Model模块中导入ResNet, ResidualBlock模型定义
...
def test_data_process():
    ...
    normalize = transforms.Normalize(mean=[0.17263485, 0.15147247, 0.14267451],
                                     std=[0.0736155, 0.06216329, 0.05930814])
...
if __name__ == '__main__':
    ...
    image = Image.open('data_test/img.png')
    normalize = transforms.Normalize(mean=[0.17263485, 0.15147247, 0.14267451],
                                     std=[0.0736155, 0.06216329, 0.05930814])
    test_transform = transforms.Compose([
        # 1. 调整图像尺寸为224x224像素（符合GoogLeNet等经典CNN网络的输入要求）
